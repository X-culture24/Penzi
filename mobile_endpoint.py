...
def process_user_input(user, user_input, phone_number):
    ...
    # DETAILS command - Returns profile details (Fixed version)
    elif user_input.startswith("DETAILS"):
        if not user:
            return "Register first with: start#name#age#gender#county#town"

        try:
            # Split the command and phone number
            ...
            response = (
                ...
                f"To express interest, SMS: {target_phone}"
            )
            return response
        except Exception as e:
            return f"Error: {str(e)}. Use: DETAILS 0712345678"
